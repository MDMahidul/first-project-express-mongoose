...
const createSemesterRegistrationIntoDB = async (
  ...
  if (ifThereAreAnyUpcomingOrOngoingSemester) {
    throw new AppError(
      ...
      `There is already a ${ifThereAreAnyUpcomingOrOngoingSemester.status} semster`,
    );
  }
...
const updateSemesterRegistrationIntoDB = async (
  ...
  // check if the semester resgistration
  const isSemesterRegistrationExists = await SemesterRegistration.findById(id);

  if (!isSemesterRegistrationExists) {
    throw new AppError(httpStatus.NOT_FOUND, 'This semester is not found');
  }
  //check if the semester reg is ended, we''ll not update that
